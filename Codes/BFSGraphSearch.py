...
class BFSGraphSearch(SearchAlgorithmBase):
    ...
    def _is_valid(self, pos):
        ...
        if 0 <= x < len(self._grid) and 0 <= y < len(self._grid[0]):
            ...
            if self._grid[x][y] != 1 and pos not in self._explored and pos not in self._frontier:  # 1 represents a wall, 2 and 3 represent start and end point
                return True
        return False

    def step(self):
        if not self._frontier: # If frontier is empty return failure
            self._done = True
            return

        current = self._frontier.popleft() # Popping the node which is in order
        self._explored.add(current) # Adding the node to explored set
        #change explpored to reached
