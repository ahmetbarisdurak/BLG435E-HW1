...
class DFSGraphSearch(SearchAlgorithmBase):
    ...
    def _is_valid(self, pos):
        ...
        if 0 <= x < len(self._grid) and 0 <= y < len(self._grid[0]):
            if self._grid[x][y] != 1 and pos not in self._explored and pos not in self._frontier:  # 1 represents a wall
                return True
        return False
    ...
    def step(self):
        ...
        current = self._frontier.pop()  # Pop from deque, which is the end of the deque
        ...
        self._explored.add(current)  # Add to explored
